<template>
  <div class="paraphrase-match">
    <!-- Glass Card -->
    <div class="group relative">
      <!-- Glow Effect -->
      <div class="absolute -inset-1 bg-gradient-to-r from-cyan-500 via-blue-500 to-indigo-500 rounded-[28px] opacity-15 group-hover:opacity-25 blur-xl transition-all duration-500"></div>
      ...
      <div :class="[
        ...
        <div class="flex items-center gap-3 mb-6">
          <span :class="[
            ...
            isDark ? 'bg-cyan-500/10 border border-cyan-500/20 text-cyan-400' : 'bg-cyan-100 border border-cyan-300 text-cyan-700'
          ]">
            改写匹配
          </span>
        </div>
        ...
        <div v-if="selectedOption" class="space-y-4">
          <div :class="[
            ...
            <p :class="['font-medium', selectedOption.isCorrect ? 'text-cyan-400' : 'text-rose-400']">
...
function getOptionClass(option) {
  if (!selectedOption.value) {
    return isDark.value
      ? 'bg-slate-700/50 border-white/10 hover:border-cyan-500/30 hover:bg-cyan-500/10'
      : 'bg-gray-50 border-gray-200 hover:border-cyan-400 hover:bg-cyan-50'
  }
